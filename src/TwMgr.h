...
struct CTwMgr
{
    ...
    void                CreateCursors();
    void                FreeCursors();
    // _Semantic/_BitmapIdx let SetCursor dispatch to the callback installed
    // via TwSetCursorCallback (see AntTweakBar.h) instead of _Cursor's
    // native platform handle; _BitmapIdx (into g_CurPict/g_CurMask, see
    // res/TwXCursors.h) is only used when _Semantic==TW_CURSOR_CUSTOM.
    void                SetCursor(CCursor _Cursor, ETwCursor _Semantic, int _BitmapIdx = -1);
